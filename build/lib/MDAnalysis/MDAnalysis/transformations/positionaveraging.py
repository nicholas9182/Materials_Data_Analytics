...
class PositionAverager(TransformationBase):
    ...
    These initial frames are typically not desired during analytics, but one can
    easily avoid them, as seen in the previous example with 
    ``if transformation.current_avg == transformation.avg_frames:`` or by
    simply removing the first ``avg_frames-1`` frames from the analytics.
    ...
    .. versionchanged:: 2.0.0
       ...
       limiting threads and checking if it can be used in parallel analytics.
    """
