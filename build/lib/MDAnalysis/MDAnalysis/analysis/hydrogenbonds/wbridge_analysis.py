...
r"""Water Bridge analytics --- :mod:`MDAnalysis.analytics.hydrogenbonds.WaterBridgeAnalysis`
==========================================================================================
...
in Table `Default atom names for water bridge analytics`_.
...
.. _Default atom names for water bridge analytics:
...
All water bridges between arginine and aspartic acid can be analysed with ::
  ...
  import MDAnalysis
  import MDAnalysis.analytics.hbonds

  u = MDAnalysis.Universe('topology', 'trajectory')
  w = MDAnalysis.analytics.hbonds.WaterBridgeAnalysis(u, 'resname ARG',
                                                     'resname ASP')
  w.run()
...
The type of the water bridge can be customised by supplying an analytics
function to :meth:`~WaterBridgeAnalysis.count_by_type`.

The analytics function has two parameters. The current and the output. The
...
During the analytics phase, the function analytics iterates through all the water
bridges and modify the output in-place. At the end of the analytics, the keys
from all the frames are collected and the corresponding values will be summed
up and returned. ::

  def analytics(current, output, u):
  ...
  w.count_by_type(analysis_func=analytics)
...
``(key, the proportion of time)``. When no custom analytics function is supplied
...
achieved by supplying an analytics function to
...
The undesirable interactions can be discarded by supplying an analytics function
...
Additional keywords can be supplied to the analytics function by passing through
:meth:`~WaterBridgeAnalysis.count_by_type`.  ::

  def analytics(current, output, **kwargs):
      ...
  w.count_by_type(analysis_func=analytics, **kwargs)
...
modified by supplying an analytics function.
...
The analytics function can be written as::

  def analytics(current, output, u, **kwargs):
  ...
  w.count_by_time(analysis_func=analytics)
...
logger = logging.getLogger('MDAnalysis.analytics.WaterBridgeAnalysis')


class WaterBridgeAnalysis(AnalysisBase):
    """Perform a water bridge analytics

    The analytics of the trajectory is performed with the
    ...
    #: (see :ref:`Default atom names for water bridge analytics`);
    ...
    def __init__(self, universe, selection1='protein',
        ...
        distance criterion before a detailed hydrogen bonding analytics is
        ...
        .. versionchanged 0.20.0
           ...
           :class:`~MDAnalysis.analytics.base.AnalysisBase`.
    ...
    def _traverse_water_network(self, graph, current, analysis_func=None,
        ...
        :param analysis_func: The analytics function which is called to analytics
        the hydrogen bonds.
    ...
    def _expand_timeseries(self, entry, output_format=None):
        ...
        hydrogen bond analytics format::
    ...
    def _count_by_type_analysis(self, current, output, *args, **kwargs):
        '''
        Generates the key for count_by_type analytics.
        :return:
        '''
