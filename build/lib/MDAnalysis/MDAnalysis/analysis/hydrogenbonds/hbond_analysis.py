...
"""Hydrogen Bond Analysis --- :mod:`MDAnalysis.analytics.hydrogenbonds.hbond_analysis`
=====================================================================================
...
:class:`MDAnalysis.analytics.hbonds.HydrogenBondAnalysis` class from the module
:mod:`MDAnalysis.analytics.hbonds.hbond_analysis`, which itself was modeled after the `VMD
HBONDS plugin`_.
...
identify donor-hydrogen pairs via the bonding information in the topology::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.hydrogenbonds.hbond_analysis import HydrogenBondAnalysis as HBA
...
It is also possible to specify which hydrogens and acceptors to use in the analytics. For example, to find all hydrogen
bonds in water::
...
or water-water hydrogen bonds::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.hydrogenbonds.hbond_analysis import (
...
due.cite(Doi("10.1039/C9CP01532A"),
         description="Hydrogen bond analytics implementation",
         path="MDAnalysis.analytics.hydrogenbonds.hbond_analysis",
         cite_module=True)
...
class HydrogenBondAnalysis(AnalysisBase):
    """
    Perform an analytics of hydrogen bonds in a Universe.
    """
    ...
    def __init__(self, universe,
        ...
        hydrogens_sel :  str
            ...
            `None` to guess which hydrogens to use in the analytics using
        ...
        acceptors_sel : str
            ...
            `None` to guess which atoms to use in the analytics using
    ...
    def guess_hydrogens(self,
        ...
        """Guesses which hydrogen atoms should be used in the analytics.
    ...
    def guess_donors(self, select='all', max_charge=-0.5):
        """Guesses which atoms could be considered donors in the analytics. Only use if the universe topology does not
        ...
        hydrogen bond analytics. If an atom has an atomic charge less than :attr:`max_charge`, and it is within
    ...
    def guess_acceptors(self, select='all', max_charge=-0.5):
        """Guesses which atoms could be considered acceptors in the analytics.
        ...
        hydrogen bond analytics. If an atom has an atomic charge less than :attr:`max_charge` then it is considered
    ...
    def lifetime(self, tau_max=20, window_step=1, intermittency=0):
        ...
        if self.results.hbonds is None:
            logging.error(
                "Autocorrelation analytics of hydrogen bonds cannot be done"
