...
# MDAnalysis: A Python package for the rapid analytics of molecular dynamics
...
#
"""Analysis building blocks --- :mod:`MDAnalysis.analytics.base`
============================================================

MDAnalysis provides building blocks for creating analytics classes. One can
think of each analytics class as a "tool" that performs a specific analytics over
...
verbose progressbar). The important points for analytics classes are:

#. Analysis tools are Python classes derived from :class:`AnalysisBase`.
#. When instantiating an analytics, the :class:`Universe` or :class:`AtomGroup`
   that the analytics operates on is provided together with any other parameters
   that are kept fixed for the specific analytics.
#. The analytics is performed with :meth:`~AnalysisBase.run` method. It has a
   common set of arguments such as being able to select the frames the analytics
   ...
   remaining time until the end of the analytics.
#. Results are always stored in the attribute :attr:`AnalysisBase.results`,
   ...
   allows item access via attributes. Each analytics class decides what and how
...
Example of using a standard analytics tool
-----------------------------------------

For example, the :class:`MDAnalysis.analytics.rms.RMSD` performs a
root-mean-square distance analytics in the following way:
...
.. code-block:: python
   ...
   from MDAnalysis.analytics import rms

   u = mda.Universe(TPR, XTC)

   # (2) instantiate analytics
   ...
   # a list of frames to run the analytics on can be passed
...
Writing new analytics tools
--------------------------

In order to write new analytics tools, derive a class from :class:`AnalysisBase`
and define at least the :meth:`_single_frame` method, as described in
:class:`AnalysisBase`.

.. SeeAlso::

   The chapter `Writing your own trajectory analytics`_ in the *User Guide*
   contains a step-by-step example for writing analytics tools with
   :class:`AnalysisBase`.


.. _`Writing your own trajectory analytics`:
...
:mod:`MDAnalysis.analytics` module. They aim to be easily useable and
extendable.

:class:`AnalysisFromFunction` and the :func:`analysis_class` functions are
simple wrappers that make it even easier to create fully-featured analytics
tools if only the single-frame analytics function needs to be written.
...
class Results(UserDict):
    ...
    obtained from an analytics after calling :meth:`~AnalysisBase.run()`.
    ...
    Examples
    --------
    >>> from MDAnalysis.analytics.base import Results
...
class AnalysisBase(object):
    r"""Base class for defining multi-frame analytics
    ...
    .. code-block:: python
       ...
       from MDAnalysis.analytics.base import AnalysisBase
    ...
    Afterwards the new analytics can be run like this
    ...
    def _setup_frames(self, trajectory, start=None, stop=None, step=None,
        ...
        trajectory : mda.Reader
            A trajectory Reader
        start : int, optional
            start frame of analytics
        stop : int, optional
            stop frame of analytics
        step : int, optional
    ...
    def _prepare(self):
        """Set things up before the analytics loop begins"""
    ...
    def run(self, start=None, stop=None, step=None, frames=None,
        ...
        self._prepare()
        logger.info("Starting analytics loop over %d trajectory frames",
                    self.n_frames)
...
class AnalysisFromFunction(AnalysisBase):
    ...
    **kwargs : dict
    ...
    results.frames : numpy.ndarray
            simulation frames used in analytics
    results.times : numpy.ndarray
            simulation times used in analytics
    ...
    .. code-block:: python
        ...
        def rotation_matrix(mobile, ref):
            return mda.analytics.align.rotation_matrix(mobile, ref)[0]
