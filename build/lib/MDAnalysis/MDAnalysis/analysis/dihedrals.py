...
#
r"""Dihedral angles analytics --- :mod:`MDAnalysis.analytics.dihedrals`
=================================================================
...
easily plotted if desired. For the :class:`~MDAnalysis.analytics.dihedrals.Ramachandran`
and :class:`~MDAnalysis.analytics.dihedrals.Janin` classes, basic plots can be
...
General dihedral analytics
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The :class:`~MDAnalysis.analytics.dihedrals.Dihedral` class is useful for calculating
...
the test data files::
   ...
   from MDAnalysis.analytics.dihedrals import Dihedral
   R = Dihedral(ags).run()

The angles can then be accessed with :attr:`Dihedral.results.angles`.

Ramachandran analytics
~~~~~~~~~~~~~~~~~~~~~

The :class:`~MDAnalysis.analytics.dihedrals.Ramachandran` class allows for the
...
:class:`~MDanalysis.analytics.dihedrals.Dihedral` class which takes a list of
...
.. Note::
   ...
   The Ramachandran analytics is prone to errors if the topology contains
...
:class:`~MDAnalysis.analytics.dihedrals.Janin` class. It works in the same way,
...
.. Note::
   ...
   The Janin analytics is prone to errors if the topology contains duplicate or
...
(:data:`~MDAnalysis.analytics.data.filenames.Rama_ref`) and Janin reference data
(:data:`~MDAnalysis.analytics.data.filenames.Janin_ref`) were made using data
...
class Dihedral(AnalysisBase):
    ...
    .. versionchanged:: 2.0.0
       ...
       :class:`MDAnalysis.analytics.base.Results` instance.
...
class Ramachandran(AnalysisBase):
    ...
    The above analytics will calculate angles from a "phi" selection of
    ...
    residues, but the analytics will still run. If a :math:`\phi` or :math:`\psi`
    selection cannot be made, that residue will be removed from the analytics.
...
class Janin(Ramachandran):
    ...
    list of residues, but the analytics will still run. Some topologies have
