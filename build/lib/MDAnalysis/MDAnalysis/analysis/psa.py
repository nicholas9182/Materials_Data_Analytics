...
r"""
Calculating path similarity --- :mod:`MDAnalysis.analytics.psa`
==========================================================================
...
import logging
logger = logging.getLogger('MDAnalysis.analytics.psa')
...
class Path(object):
    ...
    The analytics is performed with :meth:`PSAnalysis.run` and stores the result
    ...
    def __init__(self, universe, reference, select='name CA',
        ...
        select : str or dict or tuple (optional)
             ...
             2. a dictionary ``{'mobile':sel1, 'reference':sel2}`` (the
                :func:`MDAnalysis.analytics.align.fasta2select` function returns
    ...
    def fit_to_reference(self, filename=None, prefix='', postfix='_fit',
        ...
        Notes
        -----
        Uses :class:`MDAnalysis.analytics.align.AlignTraj` for the fitting.
...
class PSAnalysis(object):
    ...
    def __init__(self, universes, reference=None, select='name CA',
        ...
        # Pickle topology and trajectory filenames for this analytics to curdir
    ...
    def run(self, **kwargs):
        """Perform path similarity analytics on the trajectories to compute
        the distance matrix.
    ...
    def run_pairs_analysis(self, **kwargs):
        """Perform PSA Hausdorff (nearest neighbor) pairs analytics on all unique
        ...
        of Hausdorff pairs analytics that is available for each pair of path,
