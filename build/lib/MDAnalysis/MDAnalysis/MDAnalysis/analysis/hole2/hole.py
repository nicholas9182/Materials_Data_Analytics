...
def hole(pdbfile,
    ...
    dcd : str, optional
        ...
        matching PDB file `filename`) and then HOLE runs its analytics on
...
class HoleAnalysis(AnalysisBase):
    ...
    universe : Universe or AtomGroup
        The Universe or AtomGroup to apply the analytics to.
    select : string, optional
        The selection string to create an atom selection that the HOLE
        analytics is applied to.
    vdwradii_file : str, optional
        path to the file specifying van der Waals radii for each atom. If
        set to ``None``, then a set of default radii,
        :data:`SIMPLE2_RAD`, is used (an extension of ``simple.rad`` from
    ...
    .. versionchanged:: 2.0.0
        ...
        are now stored in a :class:`MDAnalysis.analytics.base.Results`
        instance.
    ...
    hole_header = textwrap.dedent("""
        ...
        ! written by MDAnalysis.analytics.hole2.HoleAnalysis
        ! for a Universe
        ! u = mda.Universe({}
        !   )
        ! Frame {{i}}
    ...
    def _single_frame(self):
        """Run HOLE analytics and collect profiles"""
        # set up files
        ...
        # temp pdb
        logger.info('HOLE analytics frame {}'.format(frame))
    ...
    def create_vmd_surface(self, filename='hole.vmd', dot_density=15,
        ...
        dot_density: int, optional
            ...
            original analytics. ``dot_density`` should be set between 5
