...
r"""Generating densities from trajectories --- :mod:`MDAnalysis.analytics.density`
=============================================================================
...
is suitably superimposed to provide a fixed reference frame) [#testraj]_ ::
  ...
  from MDAnalysis.analytics.density import DensityAnalysis
  u = Universe(TPR, XTC)
...
.. [#fit] Superposition can be performed with
          :class:`MDAnalysis.analytics.align.AlignTraj` or the fitting
...
logger = logging.getLogger("MDAnalysis.analytics.density")


class DensityAnalysis(AnalysisBase):
    r"""Volumetric density analytics.
    ...
    results.density : :class:`Density`
            ...
            After the analytics (see the :meth:`~DensityAnalysis.run` method),
    ...
    supplying an AtomGroup, then use  the :meth:`run` method::
        ...
        from MDAnalysis.analytics import density
        u = Universe(TPR, XTC)
        ow = u.select_atoms("name OW")
