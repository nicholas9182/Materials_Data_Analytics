...
"""Water dynamics analytics --- :mod:`MDAnalysis.analytics.waterdynamics`
=======================================================================
...
For more information about this type of analytics please refer to
...
Example use of the analytics classes
-----------------------------------
...
:meth:`MDAnalysis.analytics.hydrogenbonds.hbond_analysis.HydrogenBondAnalysis.lifetime`.

See Also
--------
:mod:`MDAnalysis.analytics.hydrogenbonds.hbond_analysis`
...
rotating/changing direction very fast::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.waterdynamics import WaterOrientationalRelaxation as WOR
...
something (residue, protein, etc)::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.waterdynamics import AngularDistribution as AD
...
water molecules, a weak rise mean slow movement of particles::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.waterdynamics import MeanSquareDisplacement as MSD
...
the zone, on the other hand, a fast decay means a short permanence time::
  ...
  import MDAnalysis
  from MDAnalysis.analytics.waterdynamics import SurvivalProbability as SP
...
other :mod:`MDAnalysis.analytics.waterdynamics` classes, which all exhibit
...
average the results. In this example, if the SP analytics were run without treating each lipid
...
logger = logging.getLogger('MDAnalysis.analytics.waterdynamics')
from MDAnalysis.lib.log import ProgressBar


class WaterOrientationalRelaxation(object):
    r"""Water orientation relaxation analytics
    ...
    t0 : int
      frame  where analytics begins
    tf : int
      frame where analytics ends
    dtmax : int
...
class AngularDistribution(object):
    r"""Angular distribution function analytics
...
class MeanSquareDisplacement(object):
    r"""Mean square displacement analytics
...
class SurvivalProbability(object):
    ...
    :mod:`MDAnalysis.analytics.waterdynamics` to support an `exclusive`
    ...
    other :mod:`MDAnalysis.analytics.waterdynamics` final frame definitions
    which are `inclusive`.
    ...
    def run(self, tau_max=20, start=None, stop=None, step=None, residues=False,
        ...
        residues : Boolean, optional
            If true, the analytics will be carried out on the residues
        ...
        .. versionchanged:: 1.0.0
           To math other analytics methods, the `stop` keyword is now exclusive
