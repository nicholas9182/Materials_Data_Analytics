...
then analytics can be sped up substantially by transferring the
...
.. rubric:: Creating an in-memory trajectory with
            :func:`~MDAnalysis.analytics.base.AnalysisFromFunction`
...
:func:`MDAnalysis.analytics.base.AnalysisFromFunction` can provide the
same functionality for any supported trajectory format::
  ...
  from MDAnalysis.coordinates.memory import MemoryReader
  from MDAnalysis.analytics.base import AnalysisFromFunction
...
to, for instance, write out a new trajectory or perform fast analytics
on the sub-system.
