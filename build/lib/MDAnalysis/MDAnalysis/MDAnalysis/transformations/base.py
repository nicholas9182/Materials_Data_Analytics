...
class TransformationBase(object):
    ...
    is history-dependent and can not be used in parallel analytics natively.
    ...
    You may need to double check if it can be used in parallel analytics;
    if not, override the value to ``False``.
    Note this attribute is not checked anywhere in MDAnalysis yet.
    Developers of the parallel analytics have to check it in their own code.
    ...
    .. versionadded:: 2.0.0
       ...
       check if it can be used in parallel analytics.
    """
