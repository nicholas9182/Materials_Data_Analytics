...
def _check_args_1(endog, n_factor, corr, nobs):
    ...
    if endog is None and corr is None:
        warnings.warn('Both endog and corr are provided, ' +
                      'corr will be used for factor analytics.')
...
class Factor(Model):
    ...
    corr : array_like
        ...
        factor analytics, it may be used in post-estimation.
    method : str
        The method to extract factors, currently must be either 'pa'
        for principal axis factor analytics or 'ml' for maximum
    ...
    **Experimental**
    ...
    .. [*] J Bai, K Li (2012).  Statistical analytics of factor models of high
    ...
    # Maximum likelihood factor analytics.
...
class FactorResults(object):
    ...
    loadings_no_rot : ndarray
        ...
        analytics.
    eigenvals : ndarray
        The eigenvalues for a factor analytics obtained using
    ...
    covariance matrix implied by the factor analytics is `GG' +
    diag(U)`.
    ...
    def summary(self):
        ...
        summ.add_title('Factor analytics results')
