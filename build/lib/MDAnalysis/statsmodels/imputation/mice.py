...
2. Once all variables have been imputed, fit the 'analytics model' to
the data set.
...
analytics model and standard errors for them.
...
'analytics model', a MICE instance can be used.  After specifying the
...
The primary goal of the analytics is usually to fit and perform
inference using an 'analytics model'. If an analytics model is not
...
* 'MICE' takes both a 'MICEData' object and an analytics model
  specification.  It runs the multiple imputation, fits the analytics
...
analytics models and results are saved.  MICEData includes a
...
class MICE(object):
    ...
    model_formula : str
        ...
        formula is for the 'analytics model'.
    model_class : statsmodels model
        The model to be fit to the imputed data sets.  This model
        class if for the 'analytics model'.
    ...
    n_skip : int
        ...
        imputed datasets that are used for analytics.
    init_kwds : dict-like
    ...
    Obtain a sequence of fitted analytics models without combining
    to obtain summary::
    %(mice_example_2)s
    ...
    def next_sample(self):
        ...
        respective imputation models, then fits the analytics model to
        the imputed data.

        Returns
        -------
        params : array_like
            The model parameters for the analytics model.

        Notes
        -----
        This function fits the analytics model and returns its
        ...
        fitting the analytics model is repeated `n_skip + 1` times and
        the analytics model parameters from the final fit are returned.
        """
    ...
    def fit(self, n_burnin=10, n_imputations=10):
        ...
        # Run without fitting the analytics model
    ...
    def combine(self):
        ...
        called.  Returns estimates and standard errors of the analytics
        model parameters.
        ...
        # The estimated parameters for the MICE analytics
        ...
        # The estimated covariance matrix for the MICE analytics
