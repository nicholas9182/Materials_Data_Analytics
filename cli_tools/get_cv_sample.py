...
def main(colvar_file: str, condition: str, traj_file: str = None, sample_size: int = 5, temperature: float = 298,
         out_group: str = "non_Water", tpr_file: str = None, output_structures: bool = False):
    """
    ...
    if output_structures:
        ...
        for i, r in sample.iterrows():
            time = r['time'] * 1000
            command = f"echo \"{out_group}\" | gmx trjconv -f {traj_file} -dump {time} -s {tpr_file} -pbc whole -o sample_{counter}.pdb"
            os.system(command)
            counter += 1
